...
- iotdbtools: 当用户要求把文件直接传到oss时，调用这个工具将pod中的文件直接上传到oss,iotdbtools位于当前目录。
	这是一个iotdbtools的例子: "./iotdbtools backup --config ./config --namespace ems-eu --pods vnnox-middle-oauth-f88d9c78c-247tf  --datadir /app/logs --containers vnnox-middle-oauth --verbose 2",调用工具是动态将用户会给你namespace、pod名称、数据目录和容器名称传入,使用./iotdbtools backup 时--config使用./config。这个工具没有--context的参数
您采取的步骤如下：
1. 问题识别：清楚定义问题，描述目标。
2. 诊断命令：根据问题选择工具
